...
local units = {
  ...
  top25       = { x = 0.00, y = 0.00, w = 1.0, h = 0.34 },
  top75       = { x = 0.00, y = 0.00, w = 1.0, h = 0.66 },
  
  bot25       = { x = 0.00, y = 0.66, w = 1.0, h = 0.34 },
  bot75       = { x = 0.00, y = 0.34, w = 1.0, h = 0.66 },
...
function obj:up() screen = hs.window.focusedWindow():screen():frame()
  window = hs.window.focusedWindow():frame()
  x = (window.x - screen.x) / screen.w 
  y = (window.y - screen.y) / screen.h 
  w = window.w / screen.w
  ...
  -- local log = hs.logger.new('mymodule','debug')
  -- log.i(window.x - screen.x, y, w, h) -- will print "[mymodule] Initializing" to the console
  if (y  == 0 and x <= 0.1 and w == 1)
  then
    if (math.abs(h - 0.5) < 0.01) 
    then
      move(units.top75, nil, true, 0) 
    elseif (math.abs(h - 0.66) < 0.01) 
    then 
      move(units.top25, nil, true, 0)
    else 
      move(units.top50, nil, true, 0)
    end
  else 
    move(units.top50, nil, true, 0)
  end
...
function obj:down() screen = hs.window.focusedWindow():screen():frame()
  window = hs.window.focusedWindow():frame()
  x = (window.x - screen.x) / screen.w 
  y = (window.y - screen.y) / screen.h 
  w = window.w / screen.w
  ...
  local log = hs.logger.new('mymodule','debug')
  log.i(x, y, w, h) -- will print "[mymodule] Initializing" to the console
  if (y - (1 - h) < 0.01 and x <= 0.1 and w == 1.0)
  then
    if (math.abs(h - 0.5) < 0.01) 
    then
      move(units.bot75, nil, true, 0)
    elseif (math.abs(h - 0.66) < 0.01) 
    then 
      move(units.bot25, nil, true, 0)
    else 
      move(units.bot50, nil, true, 0)
    end
  else 
    move(units.bot50, nil, true, 0)
  end
...
function obj:bindHotkeys(mapping)
  ...
  hs.hotkey.bind(self.mapping.sLeft[1], self.mapping.sLeft[2], function() self:previousScreen() end)
  hs.hotkey.bind(self.mapping.sRight[1], self.mapping.sRight[2], function() self:nextScreen() end)
